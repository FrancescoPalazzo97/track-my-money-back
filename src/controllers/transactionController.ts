...
export const addNewTransaction = async (req: Request, res: Response<TSuccess>) => {
    const result = TransactionInputZSchema.parse(req.body);
    await TransactionModel.insertOne(result);
    res.status(201).json({ success: true, message: 'Transazione aggiunta!' });
};
